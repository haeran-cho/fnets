...
par.lrpc <- function(object, x, eta = NULL,
  ...
  if (lrpc.adaptive) {
    ...
    Delta <- adaptive.direct.inv.est(GG, n,
      eta = eta, eta.adaptive = eta.adaptive, symmetric = "min",
      do.correct = do.correct, n.cores = n.cores
    )$DD
  } else {
    Delta <- direct.inv.est(GG,
      eta = eta, symmetric = "min",
      do.correct = do.correct, n.cores = n.cores
    )$DD
  }
...
direct.cv <- function(object, xx, target = c("spec", "acv"), symmetric = c("min", "max", "avg", "none"),
  ...
  for (fold in 1:n.folds) {
    ...
    for (ii in 1:path.length) {
      if (lrpc.adaptive) {
        DD <- adaptive.direct.inv.est(train.GG, n = n, eta = eta.path[ii], eta.adaptive = eta.adaptive, symmetric = symmetric, n.cores = n.cores)$DD
      } else {
  ...
  if (do.plot) {
    plot(eta.path, cv.err,
      type = "b", col = 2, pch = 2, log = "x",
